...
func NewAPIRouter(r *mux.Router, api API) {
	...
	publicAPI.Methods("GET", "OPTIONS").Path("/audio/tracks").HandlerFunc(api.GetAllTracks)
	publicAPI.Methods("GET", "OPTIONS").Path("/audio/tracks/{id}").HandlerFunc(api.GetTrackByID)
	publicAPI.Methods("GET", "OPTIONS").Path("/audio/playlists").HandlerFunc(api.GetAllPlaylists)
	...
	authorizedAPI.Methods("GET", "OPTIONS").Path("/audio/user-list").HandlerFunc(api.GetUserTrackList)
	...
	authorizedAPI.Methods("GET", "OPTIONS").Path("/audio/user-playlists").HandlerFunc(api.GetUserPlaylists)
